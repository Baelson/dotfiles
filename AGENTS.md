...
This repository manages a macOS developer environment using chezmoi-driven dotfiles, bootstrap scripts, and a Bats-based test suite. Follow these guidelines to contribute safely and consistently.

## Project Structure & Module Organization
- `_dotfiles/`: Source of truth for dotfiles managed by chezmoi (e.g., `dot_zshrc`, `dot_config/...`, `private_*/`, `encrypted_*.age`). Never commit decrypted secrets.
- `bootstrap/`: Machine setup and verification scripts (e.g., `setup.macos.sh`, `verify.macos.sh`).
...
- `./bootstrap.sh`: Orchestrates local bootstrap/verification flows.
...
- Chezmoi: keep `_dotfiles/` names consistent: `dot_*`, `private_*`, `encrypted_*.age`.
...
- PRs: describe intent, scope, risk, and test coverage; include logs/screenshots where helpful; call out affected paths (e.g., `_dotfiles/`, `bootstrap/`).
