...
validate_package_management() {
    ...
    if command -v brew &> /dev/null && { [ -f "$REPO_DIR/home/Brewfile" ] || [ -f "$REPO_DIR/Brewfile" ]; }; then
...
validate_chezmoi_configuration() {
    ...
    if command -v chezmoi &> /dev/null; then
        ...
        local SOURCE_DIR="$REPO_DIR/home"
        validate_item "chezmoi source directory exists" '[ -d "$REPO_DIR/home" ]'
        validate_item "chezmoi external config exists" '[ -f "$REPO_DIR/home/.chezmoiexternal.toml" ]'
        validate_item "chezmoi ignore config exists" '[ -f "$REPO_DIR/home/.chezmoiignore" ]'
